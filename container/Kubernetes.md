...
- 쿠버네티스 클러스터 안에서 파드의 집합(주로 레플리카세트)에 대한 경로나 서비스 디스커버리를 제공하는 리소스
	- *서비스 디스커버리* : api 주소가 동적으로 바뀌어도 클라이언트가 접속 대상을 바꾸지 않고 하나의 이름으로 접근할 수 있도록 하는 기능
- 서비스의 대상이 되는 파드는 서비스에서 정의하는 **레이블 셀렉터**로 정해진다

##### 서비스의 종류
서비스의 종류는 yaml 파일에 저장할 수 있다.
- 종류의 기본값 : ClusterIP 서비스
**종류**
- ClusterIP : 쿠버네티스 클러스터의 내부 IP 주소에 서비스를 공개
	- 어떤 파드에서 다른 파드 그룹으로 접근할 때 서비스를 거쳐 가도록 할 수 있다
	- 서비스명으로 네임 레졸루션이 가능
- NodePort 서비스 : 클러스터 외부에서 접근할 수 있는 서비스
	- 각 노드에서 서비스 포트로 접속하기 위한 글로벌 포트를 개방한다
- LoadBalancer 서비스 : 로컬 쿠버네티스 환경에서는 사용할 수 없는 서비스
	- 주로 각 클라우드 플랫폼에서 제공하는 로드 밸런서와 연동하기 위해 사용
- ExternalName 서비스 : 셀렉터도 포트 정의도 없는 특이 케이스
...
- Problem ☠ 
	- 쿠버네티스 클러스터 외부로 서비스를 공개하려면 서비스를 NodePort로 노출시키는데, 이 방법은 L4 레벨까지만 다룰 수 있다
	- HTTP/HTTPS처럼 경로를 기반으로 서비스를 전환하는 L7 레벨의 제어는 불가능
- 이를 해결하기 위한 리소스 -> **인그레스**
- 서비스를 이용한 쿠버네티스 클러스터 외부에 대한 노출과 가상 호스트 및 경로 기반의 정교한 HTTP 라우팅 양립 가능
